...
final public class Rule_cmdReturnVoid extends Rule
{
  ...
  public static Rule_cmdReturnVoid parse(ParserContext context)
  {
    context.push("cmdReturnVoid");
    ...
    rule = null;
    if (parsed)
      rule = new Rule_cmdReturnVoid(context.text.substring(s0, context.index), e0);
    else
      context.index = s0;

    context.pop("cmdReturnVoid", parsed);

    return (Rule_cmdReturnVoid)rule;
  }
}
