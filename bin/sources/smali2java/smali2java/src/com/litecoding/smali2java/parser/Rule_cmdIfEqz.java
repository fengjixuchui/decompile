...
final public class Rule_cmdIfEqz extends Rule
{
  private Rule_cmdIfEqz(String spelling, ArrayList<Rule> rules)
  {
  ...
  public static Rule_cmdIfEqz parse(ParserContext context)
  {
    context.push("cmdIfEqz");
    ...
    rule = null;
    if (parsed)
      rule = new Rule_cmdIfEqz(context.text.substring(s0, context.index), e0);
    else
      context.index = s0;

    context.pop("cmdIfEqz", parsed);

    return (Rule_cmdIfEqz)rule;
  }
}
