...
final public class Rule_cmdIget extends Rule
{
  private Rule_cmdIget(String spelling, ArrayList<Rule> rules)
  {
  ...
  public static Rule_cmdIget parse(ParserContext context)
  {
    context.push("cmdIget");
    ...
    {
      {
        ...
        if (parsed)
        {
          ...
          for (int i1 = 0; i1 < 1 && f1; i1++)
          {
            rule = Terminal_StringValue.parse(context, "iget");
            if ((f1 = rule != null))
            {
              e1.add(rule);
              c1++;
    ...
    rule = null;
    if (parsed)
      rule = new Rule_cmdIget(context.text.substring(s0, context.index), e0);
    else
      context.index = s0;

    context.pop("cmdIget", parsed);

    return (Rule_cmdIget)rule;
  }
}
