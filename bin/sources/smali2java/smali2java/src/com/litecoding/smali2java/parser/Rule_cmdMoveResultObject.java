...
final public class Rule_cmdMoveResultObject extends Rule
{
  ...
  public static Rule_cmdMoveResultObject parse(ParserContext context)
  {
    context.push("cmdMoveResultObject");
    ...
    {
      {
        ...
        if (parsed)
        {
          ...
          for (int i1 = 0; i1 < 1 && f1; i1++)
          {
            rule = Terminal_StringValue.parse(context, "move-result-object");
            if ((f1 = rule != null))
            {
              e1.add(rule);
              c1++;
    ...
    rule = null;
    if (parsed)
      rule = new Rule_cmdMoveResultObject(context.text.substring(s0, context.index), e0);
    else
      context.index = s0;

    context.pop("cmdMoveResultObject", parsed);

    return (Rule_cmdMoveResultObject)rule;
  }
}
