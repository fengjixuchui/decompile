...
final public class Rule_cmdInvokeDirect extends Rule
{
  ...
  public static Rule_cmdInvokeDirect parse(ParserContext context)
  {
    context.push("cmdInvokeDirect");
    ...
    {
      {
        ...
        if (parsed)
        {
          ...
          for (int i1 = 0; i1 < 1 && f1; i1++)
          {
            rule = Terminal_StringValue.parse(context, "invoke-direct");
            if ((f1 = rule != null))
            {
              e1.add(rule);
              c1++;
    ...
    rule = null;
    if (parsed)
      rule = new Rule_cmdInvokeDirect(context.text.substring(s0, context.index), e0);
    else
      context.index = s0;

    context.pop("cmdInvokeDirect", parsed);

    return (Rule_cmdInvokeDirect)rule;
  }
}
