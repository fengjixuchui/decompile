...
final public class Rule_cmdIfGtz extends Rule
{
  private Rule_cmdIfGtz(String spelling, ArrayList<Rule> rules)
  {
  ...
  public static Rule_cmdIfGtz parse(ParserContext context)
  {
    context.push("cmdIfGtz");
    ...
    rule = null;
    if (parsed)
      rule = new Rule_cmdIfGtz(context.text.substring(s0, context.index), e0);
    else
      context.index = s0;

    context.pop("cmdIfGtz", parsed);

    return (Rule_cmdIfGtz)rule;
  }
}
