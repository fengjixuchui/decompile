...
final public class Rule_cmdSgetObject extends Rule
{
  ...
  public static Rule_cmdSgetObject parse(ParserContext context)
  {
    context.push("cmdSgetObject");
    ...
    {
      {
        ...
        if (parsed)
        {
          ...
          for (int i1 = 0; i1 < 1 && f1; i1++)
          {
            rule = Terminal_StringValue.parse(context, "sget-object");
            if ((f1 = rule != null))
            {
              e1.add(rule);
              c1++;
    ...
    rule = null;
    if (parsed)
      rule = new Rule_cmdSgetObject(context.text.substring(s0, context.index), e0);
    else
      context.index = s0;

    context.pop("cmdSgetObject", parsed);

    return (Rule_cmdSgetObject)rule;
  }
}
