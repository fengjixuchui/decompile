...
final public class Rule_optPadding extends Rule
{
  ...
  public static Rule_optPadding parse(ParserContext context)
  {
    ...
    rule = null;
    if (parsed)
      rule = new Rule_optPadding(context.text.substring(s0, context.index), e0);
    else
      context.index = s0;

    context.pop("optPadding", parsed);

    return (Rule_optPadding)rule;
  }
}
